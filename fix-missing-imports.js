...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('if (currentRole === \'enfermero\') {') && lines[i+2].includes('q = query(')) {
        startIndex = i;
    }
...
if (startIndex !== -1 && endIndex !== -1) {
    const fixedBlock = `        if (currentRole === 'enfermero') {
            ...
            q = window.firebaseFirestore.query(
                window.firebaseFirestore.collection(window.firebaseFirestore.db || window.db || db, 'Bandejas_Turno'),
                window.firebaseFirestore.where('enfermeroAsignado', '==', (window.firebaseAuth || window.auth || auth).currentUser.email)
            );
        } else {
            q = window.firebaseFirestore.query(
                window.firebaseFirestore.collection(window.firebaseFirestore.db || window.db || db, 'Bandejas_Turno'),
                window.firebaseFirestore.where('estado', 'in', ['CREADA', 'EN_USO', 'CERRADA_ENFERMERIA'])
            );
        }

        unsubMisBandejas = window.firebaseFirestore.onSnapshot(q, (snapshot) => {
            ...
            snapshot.forEach(docSnap => {
                ...
                let html = \`
                    ...
                    \${data.fechaCreacion ? \`<div style="font-size:0.85em; color:#666; margin-bottom: 10px; display:flex; justify-content:space-between;">
                        <span><i class="ph ph-clock"></i> Creada: \${data.fechaCreacion?.toDate ? data.fechaCreacion.toDate().toLocaleString() : ''}</span>
