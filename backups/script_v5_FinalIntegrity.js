...
document.addEventListener('DOMContentLoaded', () => {
                ...
                try {
                    ...
                    if (docSnap.exists()) {
                        ...
                        if (typeof rawDate === 'number') {
                            rawDate = excelSerialDateToJS(rawDate).toISOString().split('T')[0];
                        }
    ...
    const moveTypeInput = document.getElementById('movimiento-tipo');

    moveTabs.forEach(btn => {
        btn.addEventListener('click', () => {
            moveTabs.forEach(b => b.classList.remove('active-green', 'active-red', 'active-orange'));
            const isRecepcion = btn.innerText.includes('RECEPCIÓN');
            
            if (isRecepcion) {
                btn.classList.add('active-green');
                moveTypeInput.value = 'entrada';
            } else {
                btn.classList.add('active-orange');
                moveTypeInput.value = 'salida';
            }
        });
    });

    // C. GUARDADO DE MOVIMIENTO (LOGICA DE NÚCLEO)
    if (moveForm) {
        moveForm.addEventListener('submit', async (e) => {
            ...
            // Sanitización de Fecha (YYYY-MM-DD -> DD / MM / AAAA)
            let rawDate = data.expirationDate;
            ...
            try {
                ...
                if (!insumoSnap.exists()) {
                    showToast('Error', 'El insumo seleccionado no existe.', 'error');
                    return;
                }

                const currentStock = insumoSnap.data().quantity || 0;

                if (type === 'salida') {
                    // VALIDACIÓN DE STOCK (Critical Rule)
                    if (quantity > currentStock) {
                        showToast('Stock Insuficiente', `Saldo actual: ${currentStock}. No es posible despachar ${quantity}.`, 'error');
                        return;
                    }
                    ...
                    // Verificamos si el lote coincide con el del insumo base o es un ingreso de nuevo lote para el mismo item?
                    // Por simplicidad del requerimiento (coincidencia Insumo + Lote):
                    if (insumoSnap.data().batch === cleanBatch) {
                        await updateDoc(insumoRef, {
                            quantity: increment(quantity),
                            updatedAt: serverTimestamp(),
                            providerId: data.providerId || insumoSnap.data().providerId
                        });
                    } else {
                        // Es un nuevo lote del mismo producto -> Crear nuevo registro (Upsert)
                        // Para evitar duplicidad excesiva, si es el mismo producto pero distinto lote, generamos un nuevo doc con el mismo name
                        await addDoc(collection(db, 'Insumos'), {
                            code: insumoSnap.data().code,
                            name: insumoSnap.data().name,
                            name_lowercase: insumoSnap.data().name.toLowerCase(),
                            quantity: quantity,
                            batch: cleanBatch,
                            expirationDate: formattedDate,
                            unitPrice: insumoSnap.data().unitPrice,
                            location: insumoSnap.data().location,
                            category: insumoSnap.data().category,
                            criticalLimit: insumoSnap.data().criticalLimit || 50,
                            updatedAt: serverTimestamp()
                        });
                    }
                }

                // REGISTRO EN HISTORIAL (Auditoría Inmutable)
                await addDoc(collection(db, 'Historial_Movimientos'), {
                    insumoName: insumoSnap.data().name,
                    quantity: quantity,
                    type: type,
                    batch: cleanBatch,
                ...
                showToast('Éxito', 'Movimiento registrado correctamente.', 'success');
                moveForm.reset();
                ...
                showToast('Error Crítico', 'Fallo en la sincronización con la base de datos.', 'error');
            } finally {
                btn.innerHTML = originalText;
                btn.disabled = false;
            }
    ...
    function startRealTimeHistorial() {
        ...
        onSnapshot(q, (snapshot) => {
            ...
            // Actualizar contador rápido de hoy
            if (countHoyEl) {
                const hoyStr = new Date().toDateString();
                const totalHoy = globalHistoryData.filter(m => m.date?.toDate().toDateString() === hoyStr).length;
                countHoyEl.innerText = totalHoy;
            }
        });

        // Filtrado Case-Insensitive (Senior Standard)
        if (searchInput) {
            searchInput.addEventListener('input', (e) => {
                const term = e.target.value.toLowerCase().trim();
                const filtered = globalHistoryData.filter(m => 
                    (m.insumoName || "").toLowerCase().includes(term) || 
    ...
    async function startRealTimeBodegas() {
        ...
        onSnapshot(collection(db, 'Bodegas'), async (bodegaSnap) => {
            globalBodegas = [];
    ...
    function renderBodegaCards(bodegas) {
        ...
        bodegas.forEach(b => {
            ...
            card.innerHTML = `
                <div class="card-title" style="color:${b.type === 'principal' ? 'var(--primary)' : 'var(--purple)'}">${b.type.toUpperCase()}</div>
                <div class="card-value" style="font-size:20px; margin: 8px 0;">${b.name}</div>
                <div class="flex-bet text-sm text-muted"><span>Cap: ${b.capacityMetrics || 'S/I'}</span><span class="badge-green">${b.isActive ? 'Activa' : 'Inactiva'}</span></div>
            `;
        ...
        btnReporteDescarte.addEventListener('click', async () => {
            ...
            try {
                ...
                snap.forEach(docSnap => {
                    ...
                    // CORRECCIÓN CRÍTICA DE FECHA (Excel Serial vs String vs Date)
                    const rawExp = d.expirationDate;
                    if (!rawExp) return;

                    if (typeof rawExp === 'number') {
                        // Es serial de Excel (ej 45321)
                        dateObj = new Date((rawExp - 25569) * 86400 * 1000);
                    } else if (typeof rawExp === 'string') {
                        // Es string "DD / MM / AAAA"
                        const parts = rawExp.replace(/\s/g, '').split('/');
                        if (parts.length === 3) {
                            dateObj = new Date(parts[2], parts[1] - 1, parts[0]);
                        }
                    } else if (rawExp.toDate) {
                        // Es Firestore Timestamp
                        dateObj = rawExp.toDate();
                    }
