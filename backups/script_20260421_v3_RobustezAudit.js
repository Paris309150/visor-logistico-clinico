...
import { getFirestore, doc, collection, runTransaction, enableIndexedDbPersistence, writeBatch, serverTimestamp, getDoc, query, where, orderBy, limit, limitToLast, startAfter, endBefore, getDocs, deleteDoc, addDoc } from "https://www.gstatic.com/firebasejs/10.9.0/firebase-firestore.js";
import { generarPlantillaExcel, procesarExcelCargaMasiva, excelSerialDateToJS } from './excelUtils.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function executeFirestoreMassiveImport(data) {
        ...
        // Función interna de sanitización (Senior Level)
        ...
        try {
            for (let i = 0; i < data.length; i += CHUNK_SIZE) {
                ...
                chunk.forEach((item, index) => {
                    try {
                        ...
                        // Solo la Descripción es obligatoria ahora
                        if (!cleanName) {
                            throw new Error("Campo 'Descripción' vacío o inválido");
                        }

                        // Si no hay ID, generamos uno temporal basado en el nombre para evitar quiebres de UI
                        const fallbackId = "AUTO-" + Math.random().toString(36).substring(2, 7).toUpperCase();
                        const finalId = item.id_producto ? item.id_producto.toString().trim() : fallbackId;

                        // Mapeo semántico sanitizado
                        const entry = {
                            code: finalId,
                            name: cleanName,
                            quantity: Number(item.cantidad) || 0,
                            unitPrice: Number(item.costo_unitario) || 0,
                            batch: sanitizeText(item.lote) || "N/A",
            ...
            // Registro en colección 'informes'
            if (failedItems.length > 0) {
                await addDoc(collection(db, 'informes'), {
                    tipo: 'Error de Carga Masiva',
                    fecha: serverTimestamp(),
                    usuario: auth.currentUser ? auth.currentUser.email : 'Admin Local',
                    total_procesado: data.length,
                    exitos: successfulItems.length,
                    errores: failedItems.length,
                    detalle_errores: failedItems.slice(0, 100)
                });
            }

            handleMassiveUploadResult(successfulItems, failedItems);
            loadFirstPage(); 

        } catch (error) {
            console.error("Carga Masiva Error Crítico:", error);
            showToast('Error Crítico', 'Fallo técnico durante la escritura masiva.', 'error');
        }
    }

    function handleMassiveUploadResult(success, failed) {
        if (failed.length === 0) {
            showToast('Éxito Total', `Se han importado ${success.length} registros sin incidencias.`, 'success');
        } else {
            const msg = `Carga Parcial: ${success.length} OK, ${failed.length} Fallos.`;
            showToast('Advertencia de Carga', msg, 'warning');
            downloadErrorReportCSV(failed); 
        }
    }
