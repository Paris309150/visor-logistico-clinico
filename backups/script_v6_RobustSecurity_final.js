...
/* ----------------------------------------------------
   ...
   ---------------------------------------------------- */
const SAR_Utils = {
    // Formateador Universal de Fechas (Excel, ISO, Timestamp)
    formatDate: (val) => {
        if (!val) return "N/A";
        let date;
        ...
        try {
            ...
            if (!date || isNaN(date.getTime())) return "Format Error";
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (moveForm) {
        moveForm.addEventListener('submit', async (e) => {
            ...
            try {
                // EJECUCIÓN ATÓMICA DE MOVIMIENTO (Transaction Recovery)
                await runTransaction(db, async (transaction) => {
                    const insumoRef = doc(db, 'Insumos', insumoId);
                    const insumoSnap = await transaction.get(insumoRef);
                    
                    if (!insumoSnap.exists()) throw "Insumo inexistente";

                    const currentStock = insumoSnap.data().quantity || 0;

                    if (type === 'salida') {
                        if (quantity > currentStock) throw "Stock insuficiente";
                        transaction.update(insumoRef, { 
