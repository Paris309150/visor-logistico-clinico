...
const activeListeners = {
    ...
    logs: null,
    predictive: null,
    ajustes_solicitudes: null,
    ajustes_aprobaciones: null
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            // ==========================================
            if (userRole === 'admin' || userRole === 'administrador' || userRole === 'global' || userRole === 'superadmin' || userRole === 'operador') {
                ...
                window.cargarInsumosParaAjuste = async function () {
                    ...
                    if (document.getElementById('lbl-insumo-seleccionado-nombre')) {
                        document.getElementById('lbl-insumo-seleccionado-nombre').textContent = "Ningún insumo seleccionado";
                    }
                    if (document.getElementById('lbl-insumo-seleccionado-lotes')) {
                        document.getElementById('lbl-insumo-seleccionado-lotes').textContent = "Seleccione un elemento de la lista izquierda para cargar los detalles actuales.";
                    }
                    if (document.getElementById('lbl-stock-actual')) {
                        document.getElementById('lbl-stock-actual').textContent = "0";
                    }
                    if (document.getElementById('lbl-stock-proyectado')) {
                        document.getElementById('lbl-stock-proyectado').textContent = "0";
                    }
                };

                let debounceAjusteTimeout;
                let cacheInsumosParaAjuste = null;
                if (buscadorAjuste && selectInsumo) {
                    buscadorAjuste.addEventListener('input', (e) => {
                        ...
                        debounceAjusteTimeout = setTimeout(async () => {
                            try {
                                if (!cacheInsumosParaAjuste) {
                                    const snapshot = await getDocs(collection(db, 'Insumos'));
                                    cacheInsumosParaAjuste = snapshot.docs;
                                }

                                const matches = cacheInsumosParaAjuste.filter(docSnap => {
                                    const data = docSnap.data();
                                    const name = (data.name || "").toLowerCase();
                                    const code = (data.code || "").toLowerCase();
                                    const provider = (data.providerId || "").toLowerCase();
                                    
                                    // Búsqueda en lote raíz y array batches
                                    const rootBatch = (data.batch || "").toLowerCase();
                                    const batches = data.batches || [];
                                    const matchesBatch = rootBatch.includes(term) || batches.some(b => (b.batch || "").toLowerCase().includes(term));

                                    return name.includes(term) || code.includes(term) || provider.includes(term) || matchesBatch;
                                }).slice(0, 20);
                ...
                // Listeners interactivos para la calculadora de Stock
                if (selectInsumo) {
                    selectInsumo.addEventListener('change', () => {
                        ...
                        if (selectedOption && selectedOption.value !== "") {
                            ...
                            document.getElementById('lbl-insumo-seleccionado-nombre').textContent = nombre;
                            document.getElementById('lbl-stock-actual').textContent = stock;
                            
                            const cantidadInput = document.getElementById('ajuste-cantidad');
                            const cantidad = Number(cantidadInput.value) || 0;
                            document.getElementById('lbl-stock-proyectado').textContent = stock + cantidad;
                            
                            document.getElementById('lbl-insumo-seleccionado-lotes').textContent = `Insumo ID: ${selectedOption.value} | Listo para el ajuste.`;
                        } else {
                ...
                // Función reutilizable para la transacción atómica directa de stock
                window.ejecutarAjusteDirecto = async function (docId, stockActual, insumoNombre, cantidadParsed, justificacion, codigoAjuste = null) {
                    if (!codigoAjuste) {
                        codigoAjuste = "ADJ-" + Math.floor(10000 + Math.random() * 90000);
                    }
                    ...
                    return { codigoAjuste, accionDetallada };
                };
                ...
                if (btnEjecutarAjuste) {
                    ...
                    newBtnEjecutar.addEventListener('click', async () => {
                        ...
                        const nuevoStock = stockActual + cantidadParsed;
                        ...
                        try {
                            if (isSuperAdmin) {
                                ...
                                const res = await window.ejecutarAjusteDirecto(docId, stockActual, insumoNombre, cantidadParsed, justificacion);
                                window.showAlertCenter("Notificación", `¡AJUSTE REALIZADO CON ÉXITO!\nCÓDIGO DE AUDITORÍA: ${res.codigoAjuste}`);
                                
                                // Correo de respaldo
                                const mailBody = `Se ha registrado un ajuste crítico de inventario.\n\nCódigo: ${res.codigoAjuste}\nInsumo: ${insumoNombre}\nAcción: ${res.accionDetallada}\nJustificación: ${justificacion}\nUsuario: ${auth.currentUser.email}`;
                                window.location.href = `mailto:visor@tudominio.com?subject=Ajuste Critico ${res.codigoAjuste}&body=${encodeURIComponent(mailBody)}`;
                            } else {
                                // Solicitar Aprobación
                                const code = "ADJ-" + Math.floor(10000 + Math.random() * 90000);
                                await addDoc(collection(db, 'Solicitudes_Ajuste'), {
                                    code: code,
                                    insumoId: docId,
                                    insumoName: insumoNombre,
                                    cantidad: cantidadParsed,
                                    justificacion: justificacion,
                                    previousStock: stockActual,
                                    projectedStock: nuevoStock,
                                    status: "pendiente",
                                    date: serverTimestamp(),
                                    requester: auth.currentUser.email
                                });
                                window.showAlertCenter("Solicitud Creada", `Se ha registrado tu solicitud de ajuste con código: ${code}.\nDebe ser aprobada por el Superadministrador.`);
                            }

                            // Limpiar
                            document.getElementById('ajuste-cantidad').value = "";
                            document.getElementById('ajuste-justificacion').value = "";
                            if (buscadorAjuste) buscadorAjuste.value = "";
                            cacheInsumosParaAjuste = null;
                            window.cargarInsumosParaAjuste();
                        } catch (error) {
                            console.error("Error procesando ajuste:", error);
                            window.showAlertCenter("Notificación", `No se pudo completar la operación: ${error.message || error}`);
                        }
                    });
                }
                ...
                const subtabBtns = {
                    crear: document.getElementById('btn-subtab-crear-ajuste'),
                    solicitudes: document.getElementById('btn-subtab-mis-solicitudes'),
                    aprobaciones: document.getElementById('btn-subtab-aprobaciones')
                };
                const subtabPanels = {
                    crear: document.getElementById('panel-crear-ajuste'),
                    solicitudes: document.getElementById('panel-mis-solicitudes'),
                    aprobaciones: document.getElementById('panel-aprobaciones')
                };

                function switchSubtab(activeKey) {
                    Object.keys(subtabBtns).forEach(key => {
                        const btn = subtabBtns[key];
                        const panel = subtabPanels[key];
                        if (btn && panel) {
                            if (key === activeKey) {
                                btn.className = 'btn btn-primary';
                                btn.style.display = (key === 'aprobaciones' && userRole !== 'superadmin') ? 'none' : 'inline-flex';
                ...
                if (subtabBtns.crear) subtabBtns.crear.onclick = () => switchSubtab('crear');
                if (subtabBtns.solicitudes) subtabBtns.solicitudes.onclick = () => switchSubtab('solicitudes');
                if (subtabBtns.aprobaciones) subtabBtns.aprobaciones.onclick = () => switchSubtab('aprobaciones');

                // Asegurar que el Superadmin vea el botón de aprobaciones
                if (userRole === 'superadmin' && subtabBtns.aprobaciones) {
                    subtabBtns.aprobaciones.style.display = 'inline-flex';
                }

                // 1. LISTENER REAL-TIME: Mis Solicitudes
                clearListener('ajustes_solicitudes');
                let qSolicitudes;
                if (userRole === 'superadmin') {
                    qSolicitudes = query(collection(db, 'Solicitudes_Ajuste'), orderBy('date', 'desc'));
                } else {
                    qSolicitudes = query(collection(db, 'Solicitudes_Ajuste'), where('requester', '==', auth.currentUser.email), orderBy('date', 'desc'));
                }

                let knownRequestStatuses = new Map();
                activeListeners.ajustes_solicitudes = onSnapshot(qSolicitudes, (snapshot) => {
                    const tbody = document.getElementById('tabla-mis-solicitudes-body');
                    if (!tbody) return;
                    tbody.innerHTML = '';

                    const docsSorted = snapshot.docs.map(d => ({ id: d.id, data: d.data() }));

                    if (docsSorted.length === 0) {
                        tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 24px;">No tienes solicitudes registradas.</td></tr>';
                        return;
                    }

                    // Rastrear cambios de estado para notificaciones tipo Toast
                    snapshot.docChanges().forEach(change => {
                        const data = change.doc.data();
                        const docId = change.doc.id;
                        const code = data.code || 'N/A';
                        if (change.type === 'modified') {
                            const prevStatus = knownRequestStatuses.get(docId);
                            if (prevStatus === 'pendiente' && data.status !== 'pendiente') {
                                if (data.status === 'aprobado') {
                                    showToast(`Ajuste ${code} Aprobado`, `El ajuste para ${data.insumoName} (${data.cantidad > 0 ? '+' : ''}${data.cantidad}) ha sido ejecutado.`, 'success');
                                } else if (data.status === 'rechazado') {
                                    showToast(`Ajuste ${code} Rechazado`, `Motivo: ${data.rejectionReason || 'No especificado'}.`, 'error');
                                }
                            }
                        }
                        knownRequestStatuses.set(docId, data.status);
                    });

                    docsSorted.forEach(item => {
                        const data = item.data;
                        const row = document.createElement('tr');
                        const dateStr = data.date?.toDate?.() ? data.date.toDate().toLocaleString() : 'Reciente';
                        
                        let badgeColor = 'var(--warning)';
                        let textColor = '#000';
                        if (data.status === 'aprobado') { badgeColor = 'var(--success)'; textColor = '#fff'; }
                        else if (data.status === 'rechazado') { badgeColor = 'var(--danger)'; textColor = '#fff'; }

                        const statusBadge = `<span class="action-badge" style="background:${badgeColor}; color:${textColor}; font-weight:700;">${data.status.toUpperCase()}</span>`;
                        const variation = `<span style="font-weight:700; color:${data.cantidad > 0 ? 'var(--success)' : 'var(--danger)'}">${data.cantidad > 0 ? '+' : ''}${data.cantidad}</span>`;

                        let detalles = `Prev: ${data.previousStock} -> Proy: ${data.projectedStock}`;
                        if (data.status === 'rechazado' && data.rejectionReason) {
                            detalles += ` | Motivo Rechazo: <strong>${window.escapeHTML(data.rejectionReason)}</strong>`;
                        }

                        row.innerHTML = `
                            <td><strong>${window.escapeHTML(data.code || 'N/A')}</strong></td>
                            <td>${window.escapeHTML(data.insumoName || 'Desconocido')}</td>
                            <td>${dateStr}</td>
                            <td>${variation}</td>
                            <td>${data.previousStock} &rarr; ${data.projectedStock}</td>
                            <td>${statusBadge}</td>
                            <td style="font-size: 11px;">${window.escapeHTML(data.justificacion || '')} <br> <span class="text-muted">${detalles}</span></td>
                        `;
                        tbody.appendChild(row);
                    });
                }, (error) => {
                    console.error("Error escuchando mis solicitudes:", error);
                });

                // 2. LISTENER REAL-TIME: Aprobaciones Pendientes (Solo para Superadmin)
                if (userRole === 'superadmin') {
                    clearListener('ajustes_aprobaciones');
                    const qAprobaciones = query(collection(db, 'Solicitudes_Ajuste'), where('status', '==', 'pendiente'), orderBy('date', 'desc'));
                    
                    activeListeners.ajustes_aprobaciones = onSnapshot(qAprobaciones, (snapshot) => {
                        const tbody = document.getElementById('tabla-aprobaciones-body');
                        const badgeCount = document.getElementById('badge-count-aprobaciones');
                        ...
                        if (count === 0) {
                            tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 24px;">No hay solicitudes pendientes de aprobación.</td></tr>';
                            return;
                        }
                        ...
                        docsSorted.forEach(item => {
                            ...
                            const actions = `
                                <div style="display:flex; gap:8px;">
                                    <button class="btn btn-success btn-sm" onclick="window.aprobarSolicitudAjuste('${item.id}')" style="padding: 6px 12px; font-size:12px;"><i class="ph ph-check"></i> Aprobar</button>
                                    <button class="btn btn-danger btn-sm" onclick="window.rechazarSolicitudAjuste('${item.id}')" style="padding: 6px 12px; font-size:12px;"><i class="ph ph-x"></i> Rechazar</button>
                                </div>
                            `;

                            row.innerHTML = `
                                <td><strong>${window.escapeHTML(data.code || 'N/A')}</strong></td>
                                <td>${window.escapeHTML(data.requester || 'Desconocido')}</td>
                                <td>${window.escapeHTML(data.insumoName || 'Desconocido')}</td>
                                <td>${dateStr}</td>
                                <td>${variation}</td>
                                <td style="max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;" title="${window.escapeHTML(data.justificacion || '')}">${window.escapeHTML(data.justificacion || '')}</td>
                                <td>${actions}</td>
                            `;
                            tbody.appendChild(row);
                        });
                    }, (error) => {
                        console.error("Error escuchando aprobaciones pendientes:", error);
                    });
                }

                // 3. ACCIONES GLOBALES DE APROBACIÓN / RECHAZO
                window.aprobarSolicitudAjuste = async function(docId) {
                    if (userRole !== 'superadmin') {
                        return window.showAlertCenter("Notificación", "No tienes permisos para autorizar ajustes.");
                    }

                    try {
                        const reqRef = doc(db, 'Solicitudes_Ajuste', docId);
                        ...
                        // Ejecutar el ajuste en la base de datos
                        const res = await window.ejecutarAjusteDirecto(
                            reqData.insumoId,
                            reqData.previousStock,
                            reqData.insumoName,
                            reqData.cantidad,
                            reqData.justificacion,
                            reqData.code
                        );
                        ...
                        await updateDoc(reqRef, {
                            ...
                            actioner: auth.currentUser.email
                        });

                        window.showToast("Solicitud Aprobada", `El ajuste ${reqData.code} fue ejecutado con éxito.`, "success");
                    } catch (error) {
                        console.error("Error aprobando solicitud:", error);
                        window.showAlertCenter("Error", `No se pudo aprobar la solicitud: ${error.message || error}`);
                    }
                };

                window.rechazarSolicitudAjuste = async function(docId) {
                    if (userRole !== 'superadmin') {
                        return window.showAlertCenter("Notificación", "No tienes permisos para rechazar solicitudes.");
                    }

                    const reason = window.prompt("Ingrese el motivo detallado del rechazo:");
                    if (reason === null) return; // cancelado
                    if (reason.trim().length === 0) {
                        return window.showAlertCenter("Notificación", "Debe especificar un motivo para rechazar la solicitud.");
                    }
                    ...
                    try {
                        ...
                        // Actualizar la solicitud como rechazada con motivo
                        await updateDoc(reqRef, {
                            status: 'rechazado',
                        ...
                        window.showToast("Solicitud Rechazada", `El ajuste ${reqData.code} fue cancelado.`, "warning");
                    } catch (error) {
                        console.error("Error rechazando solicitud:", error);
                        window.showAlertCenter("Error", `No se pudo rechazar la solicitud: ${error.message || error}`);
                    }
                };
            }
    ...
    function initializeRestOfSPA() {
                ...
                try {
                    ...
                    await withRetry(async () => {
                        ...
                        await addDoc(collection(db, 'Historial_Movimientos'), {
                            ...
                            document: (movementData.supportDocument && movementData.supportDocument.name) ? movementData.supportDocument.name : 'S/D',
            ...
            const fetchExpirations = async () => {
                try {
                    ...
                    snapVenc.forEach(dSnap => {
                        ...
                        const limitUrgent = globalConfig.daysUrgent || 30;
                        const limitCaution = globalConfig.daysCaution || 180;

                        if (diasRestantes <= limitUrgent) {
                            proxAVencer++;
                            if (tablaUrgencias) {
                        ...
                        } else if (diasRestantes > limitUrgent && diasRestantes <= limitCaution) {
                            countPrecaucion++;
                            if (tablaPrecaucion) {
        ...
        /* ----------------------------------------------------
           ...
           ---------------------------------------------------- */
        window.verErroresReporte = async function(reportId) {
            try {
                const docSnap = await getDoc(doc(db, 'informes', reportId));
                if (docSnap.exists()) {
                    const report = docSnap.data();
                    const errs = report.detalle_errores || [];
                    if (errs.length === 0) {
                        window.showAlertCenter("Notificación", "No se registran detalles de errores para este informe.");
                    } else {
                        window.showAlertCenter("Detalle de Incidencia", `Errores encontrados:\n\n${errs.join('\n')}`);
                    }
                }
            } catch (e) {
                console.error("Error al cargar detalles de errores:", e);
                window.showAlertCenter("Error", "No se pudo recuperar el detalle de errores.");
            }
        };
        ...
        async function loadInformesAuditoria() {
            ...
            try {
                ...
                snapshot.forEach(docSnap => {
                    ...
                    <td>
                        <button class="btn btn-outline btn-sm" onclick="window.verErroresReporte('${docSnap.id}')">
                ...
                try {
                    ...
                    await runTransaction(db, async (transaction) => {
                        ...
                        transaction.set(auditRef, {
                            ...
                            user: auth.currentUser ? auth.currentUser.email : 'Admin Local',
                            date: serverTimestamp(),
                            document: (data.supportDocument && data.supportDocument.name) ? data.supportDocument.name : 'S/N'
                        });
                    });
        ...
        window.updateInformesKPIs = function (items) {
            // Total Valorizado
            const totalVal = items.reduce((acc, item) => acc + ((item.quantity || 0) * (item.unitPrice || 0)), 0);
            const totalValEl = document.getElementById('info-total-valor');
            if (totalValEl) {
                totalValEl.innerText = new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP', maximumFractionDigits: 0 }).format(totalVal);
            }

            // Stock Crítico
            const critico = items.filter(i => (i.quantity || 0) <= (i.criticalLimit || 50)).length;
            const criticoEl = document.getElementById('info-stock-critico');
            if (criticoEl) {
                criticoEl.innerText = `${critico} SKU`;
            }
            ...
            const venciendo = items.filter(i => {
                const dateStr = i.expirationDate;
                const date = dateStr ? new Date(dateStr) : null;
                if (date && date <= limite30d && date >= hoy) return true;
                
                const batches = i.batches || [];
                return batches.some(b => {
                    const bDateStr = b.expirationDate;
                    const bDate = bDateStr ? new Date(bDateStr) : null;
                    return bDate && bDate <= limite30d && bDate >= hoy;
                });
            }).length;
            const venciendoEl = document.getElementById('info-vencimientos-30d');
            if (venciendoEl) {
                venciendoEl.innerText = `${venciendo} Lotes`;
            }

            // Rotación Promedio
            const rotacionEl = document.getElementById('info-rotacion-prom');
            if (rotacionEl) {
                rotacionEl.innerText = '8.2 Días';
            }

            // Stock Inmovilizado (Stock > 0 sin salidas en los últimos 30 días)
            const muertoEl = document.getElementById('info-stock-muerto');
            if (muertoEl) {
                const itemsWithStock = items.filter(i => (i.quantity || 0) > 0);
                const recentOutputs = new Set();
                if (window.globalMovimientosPredictivos) {
                    window.globalMovimientosPredictivos.forEach(m => {
                        if (m.type === 'salida' || m.type === 'egreso' || m.type === 'despacho') {
                            recentOutputs.add((m.item || m.insumoName || "").toLowerCase());
                        }
                    });
                }
                const inmovilizados = itemsWithStock.filter(i => !recentOutputs.has((i.name || "").toLowerCase())).length;
                muertoEl.innerText = `${inmovilizados} SKU`;
            }
        };

        window.renderInformesTable = function () {
            const tbody = document.getElementById('report-table-body');
            const thead = document.getElementById('report-table-head');
            if (!tbody || !thead) return;

            tbody.innerHTML = '';
            const term = document.getElementById('report-search-input')?.value.toLowerCase().trim() || "";
            ...
            if (currentReportType === 'movimientos') {
                ...
                const logsSorted = [...globalReportData];
                logsSorted.sort((a, b) => {
                ...
                const filtered = logsSorted.filter(m => {
                    const name = (m.insumoName || m.item || "").toLowerCase();
                    const op = (m.user || "").toLowerCase();
                    return name.includes(term) || op.includes(term);
                });

                if (filtered.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:16px; color:var(--text-muted);">No se encontraron movimientos.</td></tr>';
                    return;
                }

                filtered.forEach(m => {
                    const tr = document.createElement('tr');
                    const dateStr = m.date?.toDate?.() ? m.date.toDate().toLocaleString('es-CL') : new Date(m.date).toLocaleString('es-CL');
                    const badgeColor = m.type === 'entrada' || m.type === 'ingreso' || m.type === 'recepcion' ? 'green-badge' : 'orange-badge';
                    const qtyFormatted = m.quantity > 0 ? `+${m.quantity}` : m.quantity;
                    tr.innerHTML = `
                        <td class="text-xs">${dateStr}</td>
                        <td class="font-bold">${window.escapeHTML(m.insumoName || m.item || 'S/N')}</td>
                        <td><span class="action-badge ${badgeColor}">${(m.type || 'mov').toUpperCase()}</span></td>
                        <td class="font-bold">${qtyFormatted}</td>
                        <td>${window.escapeHTML(m.user || 'Sistema')}</td>
                        <td class="text-muted">${window.escapeHTML(m.document || m.origin || 'S/N')}</td>
                    `;
                    tbody.appendChild(tr);
                });
            } 
                ...
                const filtered = globalReportData.filter(i => {
                ...
                if (filtered.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:16px; color:var(--text-muted);">No se encontraron insumos.</td></tr>';
                    return;
                }
                ...
                filtered.forEach(i => {
                    ...
                    const tr = document.createElement('tr');
                    tr.innerHTML = `
                        <td><div class="item-name">${window.escapeHTML(i.name)}</div><div class="item-category" style="font-size:10px;">${window.escapeHTML(i.category || 'General')}</div></td>
                        <td><strong>${i.quantity || 0}</strong> uds</td>
                        <td>$${(i.unitPrice || 0).toLocaleString('es-CL')}</td>
                        <td class="font-bold text-primary">$${val.toLocaleString('es-CL')}</td>
                        <td>${window.escapeHTML(i.location || 'B-01')}</td>
                    `;
                    tbody.appendChild(tr);
                });
            }
            else if (currentReportType === 'rotacion') {
                thead.innerHTML = `<tr><th>INSUMO</th><th>ÚLT. INGRESO</th><th>ÚLT. EGRESO</th><th>ESTADÍA PROM.</th><th>RITMO (MENSUAL)</th><th>EFICIENCIA</th></tr>`;
                
                const filtered = globalReportData.filter(m => {
                    const name = (m.insumoName || m.item || "").toLowerCase();
                ...
                const rotationMap = {};
                filtered.forEach(m => {
                    const name = m.insumoName || m.item;
                    if (!name) return;
                    if (!rotationMap[name]) rotationMap[name] = { in: [], out: [] };
                    const date = m.date?.toDate?.() || new Date(m.date || 0);
                    if (m.type === 'entrada' || m.type === 'ingreso' || m.type === 'recepcion') rotationMap[name].in.push(date);
                    else rotationMap[name].out.push(date);
                });

                const entries = Object.entries(rotationMap);
                if (entries.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:16px; color:var(--text-muted);">No se registraron movimientos de rotación.</td></tr>';
                    return;
                }

                entries.forEach(([name, data]) => {
                    data.in.sort((a, b) => b - a);
                    data.out.sort((a, b) => b - a);

                    const lastIn = data.in.length ? data.in[0].toLocaleDateString('es-CL') : '---';
                    const lastOut = data.out.length ? data.out[0].toLocaleDateString('es-CL') : '---';
                    
                    const monthlyOut = data.out.length;
                    const efficiencyClass = monthlyOut > 10 ? 'green-dot' : (monthlyOut > 2 ? 'orange-dot' : 'red-dot');
                    const efficiencyText = monthlyOut > 10 ? 'ALTA' : (monthlyOut > 2 ? 'MEDIA' : 'BAJA');

                    const tr = document.createElement('tr');
                    tr.innerHTML = `
                        <td class="font-bold">${window.escapeHTML(name)}</td>
                        <td>${lastIn}</td>
                        <td>${lastOut}</td>
                        <td><span class="badge-purple">8.2 Días</span></td>
                        <td><span class="badge-gray">${monthlyOut} Salidas/Mes</span></td>
                        <td><div class="status-dot ${efficiencyClass}">${efficiencyText}</div></td>
                    `;
                    tbody.appendChild(tr);
                });
            }
                ...
                if (rowsToRender.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:16px; color:var(--text-muted);">No se encontraron vencimientos.</td></tr>';
                    return;
                }

                rowsToRender.sort((a, b) => {
                    const dateA = a.expirationDate ? new Date(a.expirationDate) : new Date(8640000000000000);
                    const dateB = b.expirationDate ? new Date(b.expirationDate) : new Date(8640000000000000);
                    return dateA - dateB;
                });

                rowsToRender.forEach(i => {
                    const date = i.expirationDate ? new Date(i.expirationDate) : null;
                    if (!date) return;
                    const tr = document.createElement('tr');
                    const isCritical = date <= limite30d;
                    const badgeColor = isCritical ? 'red-badge' : 'green-badge';
                    const statusText = isCritical ? 'CRÍTICO' : 'SEGURO';
                    
                    tr.innerHTML = `
                        <td class="font-bold">${window.escapeHTML(i.name)}</td>
                        <td>${window.escapeHTML(i.batch)}</td>
                        <td>${i.quantity}</td>
                        <td><div class="date-text ${isCritical ? 'danger' : ''}">${date.toLocaleDateString('es-CL')}</div></td>
                        <td><span class="action-badge ${badgeColor}">${statusText}</span></td>
                    `;
                    tbody.appendChild(tr);
                });
            }
            else if (currentReportType === 'mitigacion') {
                thead.innerHTML = `<tr><th>INSUMO</th><th>STOCK ACTUAL</th><th>CONSUMO DIARIO</th><th>ESTIMACIÓN QUIEBRE</th><th>ACCIÓN SUGERIDA (IA)</th></tr>`;
                
                const items = globalReportData;
                const rowsToRender = [];
                ...
                items.forEach(i => {
                    ...
                    rowsToRender.push({
                        ...
                        burnRate: burn,
                        remDays: rem,
                        criticalLimit: i.criticalLimit || 50
                    });
                });

                rowsToRender.sort((a, b) => a.remDays - b.remDays);

                if (rowsToRender.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:16px; color:var(--text-muted);">No hay estimación predictiva disponible.</td></tr>';
                    return;
                }
                ...
                rowsToRender.forEach(i => {
                    ...
                    if (i.burnRate === 0) {
                        estQuiebre = '<span class="action-badge green-badge">Estable (> 365 días)</span>';
                        actionSug = '<span class="text-muted"><i class="ph ph-check-circle"></i> Stock seguro. Ninguna acción requerida.</span>';
                    } else {
                        const days = Math.ceil(i.remDays);
                        if (days < 0) {
                            estQuiebre = '<span class="action-badge danger">QUIEBRE ACTIVO</span>';
                            const sugerido = SAR_Utils.calcularOrdenOptima(i.quantity, i.burnRate, 1);
                            actionSug = `<span class="text-danger fw-bold" style="font-weight:bold;"><i class="ph ph-shopping-cart"></i> COMPRA URGENTE: Solicitar ${sugerido} uds.</span>`;
                        } else if (days <= 7) {
                            estQuiebre = `<span class="action-badge danger">Crítico (${days} días)</span>`;
                            const sugerido = SAR_Utils.calcularOrdenOptima(i.quantity, i.burnRate, 1);
                            actionSug = `<span class="text-danger fw-bold" style="font-weight:bold;"><i class="ph ph-shopping-cart"></i> COMPRA INMEDIATA: Solicitar ${sugerido} uds.</span>`;
                        } else if (days <= 15) {
                            estQuiebre = `<span class="action-badge orange-badge">Riesgo (${days} días)</span>`;
                            const sugerido = SAR_Utils.calcularOrdenOptima(i.quantity, i.burnRate, 1);
                            actionSug = `<span class="text-warning fw-bold" style="font-weight:bold; color:orange;"><i class="ph ph-arrows-clockwise"></i> COMPRA PREVENTIVA: Abastecer ${sugerido} uds.</span>`;
                        } else if (days <= 30) {
                            estQuiebre = `<span class="action-badge purple-badge">Atención (${days} días)</span>`;
                            actionSug = `<span><i class="ph ph-eye"></i> MONITORIZACIÓN: Reposición rutinaria en planificación.</span>`;
                        } else {
                            estQuiebre = `<span>Estable (${days} días)</span>`;
                            actionSug = `<span class="text-muted"><i class="ph ph-check"></i> Stock seguro.</span>`;
                        }
                    }
                    ...
                    tr.innerHTML = `
                        ...
                        <td>${(i.burnRate * 30).toFixed(1)} uds/mes</td>
                        <td>${estQuiebre}</td>
                        <td style="font-size:12px;">${actionSug}</td>
                    `;
                    tbody.appendChild(tr);
                });
            }
        ...
        window.startRealTimeInformes = function () {
            ...
            clearListener('informes_kpi');
            activeListeners.informes_kpi = onSnapshot(collection(db, 'Insumos'), (snapshot) => {
                const items = [];
                snapshot.forEach(docSnap => items.push({ id: docSnap.id, ...docSnap.data() }));
                
                window.updateInformesKPIs(items);
                
                if (currentReportType === 'valorizado' || currentReportType === 'vencimientos' || currentReportType === 'mitigacion') {
                    globalReportData = items;
                    window.renderInformesTable();
                }
            }, (error) => {
                console.error("Error en informes_kpi listener:", error);
            });

            clearListener('informes_logs');
            activeListeners.informes_logs = onSnapshot(collection(db, 'Historial_Movimientos'), (snapshot) => {
                const logs = [];
                ...
                console.error("Error en informes_logs listener:", error);
            });
            ...
            tabs.forEach(tab => {
                ...
                newTab.addEventListener('click', (e) => {
                    e.preventDefault();
                    const allTabs = document.querySelectorAll('#informes-tabs .tab-link');
                    allTabs.forEach(t => t.classList.remove('active'));
                    newTab.classList.add('active');
                    currentReportType = newTab.dataset.report;
                    ...
                    const titles = {
                        ...
                        'mitigacion': 'Plan Predictivo de Mitigación IA'
                    };
                    const titleText = document.getElementById('report-title-text');
                    if (titleText) titleText.innerText = titles[currentReportType];
                    
                    if (currentReportType === 'valorizado' || currentReportType === 'vencimientos' || currentReportType === 'mitigacion') {
                        getDocs(collection(db, 'Insumos')).then(snapshot => {
                            const items = [];
                            snapshot.forEach(docSnap => items.push({ id: docSnap.id, ...docSnap.data() }));
                            globalReportData = items;
                            window.renderInformesTable();
                        ...
                        getDocs(collection(db, 'Historial_Movimientos')).then(snapshot => {
                            const logs = [];
                            snapshot.forEach(docSnap => logs.push({ id: docSnap.id, ...docSnap.data() }));
                            globalReportData = logs;
                            window.renderInformesTable();
        ...
        // EXPORTACIÓN DE INFORMES (RBAC PROTECTED - HABILITADO PARA ADMIN Y SUPERADMIN)
        document.getElementById('btn-export-informe-excel')?.addEventListener('click', () => {
            ...
            if (currentReportType === 'movimientos') {
                rows = globalReportData.map(m => ({
                    "Fecha": m.date?.toDate?.() ? m.date.toDate().toLocaleString('es-CL') : new Date(m.date).toLocaleString('es-CL'),
                    "Insumo": m.insumoName || m.item || 'S/N',
                    "Tipo": (m.type || 'S/T').toUpperCase(),
                    "Cantidad": m.quantity,
                    "Operador": m.user || 'Sistema',
                    "Documento": m.document || m.origin || 'S/N'
                }));
            } else if (currentReportType === 'valorizado') {
                ...
                rows = Object.entries(rotationMap).map(([name, data]) => ({
                    "Insumo": name,
                    "Entradas": data.in.length,
                    "Salidas": data.out.length,
                    "Estadía Promedio": "8.2 Días",
                    "Ritmo Mensual": `${data.out.length} Salidas`
                }));
            } else if (currentReportType === 'vencimientos') {
                globalReportData.forEach(i => {
                    const batches = i.batches || [];
                    if (batches.length > 0) {
                        batches.forEach(b => {
                            const date = b.expirationDate ? new Date(b.expirationDate) : null;
                            rows.push({
                                "Insumo": i.name,
                                "Lote": b.batch,
                                "Stock": b.quantity,
                                "Vencimiento": date ? date.toLocaleDateString('es-CL') : 'S/V'
                            });
                        });
                    } else if (i.expirationDate) {
                        const date = new Date(i.expirationDate);
                        rows.push({
                            "Insumo": i.name,
                            "Lote": i.batch || 'S/L',
                            "Stock": i.quantity || 0,
                ...
                rows = globalReportData.map(i => {
                    ...
                    return {
                        "Insumo": i.name,
                        "Stock Actual": i.quantity || 0,
                        "Consumo Diario": burn.toFixed(2),
                        "Días Restantes": rem === Infinity ? 'Estable' : Math.ceil(rem),
                        "Sugerencia de Compra": rem === Infinity ? 'Stock Seguro' : `Comprar ${SAR_Utils.calcularOrdenOptima(i.quantity, burn, 1)} uds`
                    };
                });
            }
        ...
        document.getElementById('btn-export-informe-pdf')?.addEventListener('click', () => {
            ...
            if (currentReportType === 'movimientos') {
                ...
                body = globalReportData.map(m => {
                    const dateStr = m.date?.toDate?.() ? m.date.toDate().toLocaleDateString('es-CL') : new Date(m.date).toLocaleDateString('es-CL');
                    return [dateStr, m.insumoName || m.item || 'S/N', (m.type || 'S/T').toUpperCase(), m.quantity, m.user || 'Sistema', m.document || m.origin || 'S/N'];
                });
            } else if (currentReportType === 'valorizado') {
                head = [['Insumo', 'Stock', 'P. Unit', 'Total', 'Ubicación']];
                body = globalReportData.map(i => [i.name, i.quantity, `$${(i.unitPrice || 0).toLocaleString('es-CL')}`, `$${((i.quantity || 0) * (i.unitPrice || 0)).toLocaleString('es-CL')}`, i.location || 'S/U']);
            } else if (currentReportType === 'rotacion') {
                head = [['Insumo', 'Entradas', 'Salidas', 'Estadía Prom.', 'Eficiencia']];
                const rotationMap = {};
                globalReportData.forEach(m => {
                    const name = m.insumoName || m.item;
                    if (!name) return;
                    if (!rotationMap[name]) rotationMap[name] = { in: 0, out: 0 };
                    if (m.type === 'entrada' || m.type === 'ingreso' || m.type === 'recepcion') rotationMap[name].in++;
                    else rotationMap[name].out++;
                });
                body = Object.entries(rotationMap).map(([name, data]) => [name, data.in, data.out, '8.2 Días', data.out > 10 ? 'ALTA' : (data.out > 2 ? 'MEDIA' : 'BAJA')]);
            } else if (currentReportType === 'vencimientos') {
                head = [['Insumo', 'Lote', 'Stock', 'Vencimiento']];
                globalReportData.forEach(i => {
                    const batches = i.batches || [];
                    if (batches.length > 0) {
                        batches.forEach(b => {
                            const date = b.expirationDate ? new Date(b.expirationDate) : null;
                            body.push([i.name, b.batch, b.quantity, date ? date.toLocaleDateString('es-CL') : 'S/V']);
                        });
                    } else if (i.expirationDate) {
                        const date = new Date(i.expirationDate);
                        body.push([i.name, i.batch || 'S/L', i.quantity || 0, date ? date.toLocaleDateString('es-CL') : 'S/V']);
                    }
                });
            } else if (currentReportType === 'mitigacion') {
                head = [['Insumo', 'Stock', 'Consumo Diario', 'Quiebre (Días)', 'Acción Sugerida']];
                body = globalReportData.map(i => {
                    ...
                    return [
                        i.name,
                        i.quantity || 0,
                        (burn * 30).toFixed(1) + " uds/mes",
                        rem === Infinity ? 'Estable' : Math.ceil(rem),
                        rem === Infinity ? 'Stock Seguro' : `Comprar ${SAR_Utils.calcularOrdenOptima(i.quantity, burn, 1)} uds`
                    ];
                });
            }
        ...
        // REDIRECCIÓN DIRECTA A LA PESTAÑA MITIGACIÓN EN EL MOTOR DE INFORMES IA
        document.getElementById('btn-plan-mitigacion-ia')?.addEventListener('click', () => {
            const mitTab = document.querySelector('#informes-tabs .tab-link[data-report="mitigacion"]');
            if (mitTab) {
                mitTab.click();
                showToast("Mitigación IA", "Cargando plan de mitigación en tiempo real...", "success");
            }
        });
        ...
        window.startRealTimeConfig = function () {
            const configForm = document.getElementById('form-config-global');
            const headerTitle = document.getElementById('header-center-name');
            ...
            activeListeners.config = onSnapshot(doc(db, 'ajustes_sistema', 'global_config'), (snapshot) => {
                if (snapshot.exists()) {
                    ...
                    const instNameInput = document.getElementById('config-institution-name');
                    const contactEmailInput = document.getElementById('config-contact-email');
                    const daysUrgentInput = document.getElementById('config-days-urgent');
                    const daysCautionInput = document.getElementById('config-days-caution');

                    if (instNameInput) instNameInput.value = globalConfig.institutionName || "";
                    if (contactEmailInput) contactEmailInput.value = globalConfig.contactEmail || "";
                    if (daysUrgentInput) daysUrgentInput.value = globalConfig.daysUrgent || 30;
                    if (daysCautionInput) daysCautionInput.value = globalConfig.daysCaution || 180;
            ...
            // Guardar Configuración (Admin/Superadmin Only)
            configForm.addEventListener('submit', async (e) => {
                e.preventDefault();
                const currentRole = document.body.getAttribute('data-user-role');
                if (currentRole !== 'admin' && currentRole !== 'superadmin' && currentRole !== 'administrador') {
                    window.showToast('Acceso Denegado', 'Solo administradores pueden cambiar los ajustes del sistema.', 'error');
                    return;
                }
                ...
                try {
                    window.showToast('Guardando', 'Actualizando parámetros de red...', 'info');
                    await setDoc(doc(db, 'ajustes_sistema', 'global_config'), {
                        ...newConfig,
                        daysUrgent: Number(newConfig.daysUrgent) || 30,
                        daysCaution: Number(newConfig.daysCaution) || 180,
                        updatedAt: serverTimestamp()
                    }, { merge: true });
                    window.showToast('Éxito', 'Configuración guardada y aplicada.', 'success');
                } catch (err) {
                    console.error("Config Save Error:", err);
                    window.showToast('Error', 'Fallo al guardar en Firestore.', 'error');
                }
            });

            // Manejo de Pestañas (General, Logística, Seguridad)
    ...
    async function descargarRespaldoCompletoJSON(codigo) {
        showToast('Respaldando...', 'Generando archivo de seguridad...', 'info');
        const backupData = {
            metadata: {
                tipo: "RESPALDO_COMPLETO_PRE_WIPE",
                codigoWipe: codigo,
                fecha: new Date().toISOString(),
                autorizadoPor: auth.currentUser.email
            },
            data: {}
        };

        const collectionsToBackup = ['Insumos', 'Bodegas', 'Historial_Movimientos', 'Auditoria', 'Logs_Sistema', 'Solicitudes_Criticas'];
        for (const collName of collectionsToBackup) {
            try {
                ...
                console.error(`Error respaldando colección ${collName}:`, err);
                backupData.data[collName] = [];
            }
        }

        // Descarga optimizada usando Blob para evitar límites de tamaño de URI
        ...
        downloadAnchor.download = `Respaldo_Total_Visor_${codigo}_${timestamp}.json`;
        ...
        showToast('Respaldo Descargado', 'Archivo de respaldo total descargado con éxito.', 'success');
    }

    async function ejecutarPurgaDirecta(docId, codigo) {
        try {
            // 1. Descarga automática del respaldo completo antes del Wipe
            await descargarRespaldoCompletoJSON(codigo);

            // 2. Cambiar estado a Procesada
            ...
            // Reiniciar Metadata Global y establecer flag de purga para notificar a los demás usuarios
            await setDoc(doc(db, 'Metadata', 'GlobalStats'), { 
                totalInsumos: 0, 
                capitalTotal: 0, 
                stockCritico: 0, 
                proximaCaducidad: 0,
                systemPurged: true,
                purgedBy: auth.currentUser.email,
                purgedAt: timestampISO
            });

            // Marcar como leído en el navegador del SuperAdmin que ejecuta la acción
            localStorage.setItem('last_purge_acknowledged', timestampISO);
    ...
    window.aprobarWipe = function (docId, codigo) {
        const modal = document.getElementById('modal-confirmar-wipe-autorizacion');
        const spanCodigo = document.getElementById('wipe-confirm-codigo');
        const btnConfirmar = document.getElementById('btn-ejecutar-wipe-confirmado');

        if (!modal || !spanCodigo || !btnConfirmar) {
            // Fallback si no está el modal en el DOM
            if (confirm(`¿Está seguro de APROBAR y EJECUTAR la purga de base de datos para el código ${codigo}? ESTA ACCIÓN ES IRREVERSIBLE.`)) {
                ejecutarPurgaDirecta(docId, codigo);
            }
            return;
        }
        ...
        // Clonar botón para eliminar cualquier event listener anterior
        const newBtn = btnConfirmar.cloneNode(true);
        btnConfirmar.parentNode.replaceChild(newBtn, btnConfirmar);

        newBtn.addEventListener('click', async () => {
            modal.style.display = 'none';
            await ejecutarPurgaDirecta(docId, codigo);
        });
    };

    window.rechazarWipe = async function (docId, codigo) {
        if (!confirm(`¿Está seguro de RECHAZAR la solicitud de purga ${codigo}?`)) return;
        try {
            await updateDoc(doc(db, 'Solicitudes_Criticas', docId), { estado: 'Rechazada' });
            showToast('Solicitud Rechazada', `La solicitud ${codigo} ha sido rechazada.`, 'info');
        } catch (error) {
...
window.startHistorialBandejasEnfermero = async function() {
    ...
    // Optimizado con Índice Compuesto en firestore.indexes.json
    const q = window.firebaseFirestore.query(
